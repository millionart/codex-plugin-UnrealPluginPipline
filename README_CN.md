...
如果要在 Codex Desktop 之外使用 Codex 代理构建模式，还需要安装独立 Codex CLI：
...
Codex Desktop 自带的 WindowsApps 资源不是可用于此流水线的 `codex exec` CLI。在 Codex Desktop 内部，`build` 会检测桌面会话，并把发布循环保留在当前对话里，而不是再启动一个嵌套 Codex 会话。如果只想直接调用 Unreal 打包，使用 `build-only` 即可。
...
在 Codex Desktop 之外，`Build` 代理模式会启动：
...
在 Codex Desktop 内部，`Build` 不会启动第二个 Codex 进程。它会在当前对话可见的 release loop 中运行，写入 `<output-directory>\reports\last-release-report.md`，并在某个引擎版本失败时停下，让当前 Codex 代理读取日志、修复项目、重试并继续。
